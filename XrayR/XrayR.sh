...
install() {
    bash <(curl -Ls https://github.moeyy.xyz/https://raw.githubusercontent.com/YouRan_Proxy/master/XrayR/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://github.moeyy.xyz/https://raw.githubusercontent.com/YouRan_Proxy/master/XrayR/install.sh) $version
    if [[ $? == 0 ]]; then
...
install_bbr() {
    bash <(curl -L -s https://github.moeyy.xyz/https://raw.githubusercontent.com/chiakge/Linux-NetSpeed/master/tcp.sh)
    #if [[ $? == 0 ]]; then
    #    echo ""
